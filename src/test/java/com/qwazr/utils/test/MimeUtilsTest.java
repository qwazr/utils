...
public class MimeUtilsTest {
	...
	public void extract() throws IOException, FileUploadException {
		final InputStream input = MimeUtilsTest.class.getResourceAsStream(MULTIPART_TEST);
		final File tmpDirectory = Files.createTempDirectory(MimeUtilsTest.class.getName()).toFile();
		final MimeUtils.StreamFileUpload parser = new MimeUtils.StreamFileUpload(tmpDirectory);
